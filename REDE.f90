...
!        IF (IC==6273) THEN
!        WRITE(*,*) 'IT, IC, QCEL2 = ', IT, IC, QCEL2(IC)
!        PAUSE
!        ENDIF
				...
				IF(IS>0)THEN
					...
					ELSE !use flow from file
						...
						DO LT=1,NTMUSK+1 !Replaces calculated flow by read flow (from file)
							QCONTORM(JUS,LT)=QLIDO(IS,IT-1)+(LT-1)*(QLIDO(IS,IT)-QLIDO(IS,IT-1))/NTMUSK+QCONTORM(JUS,LT)!Correção walter replicada LBF 2019
						ENDDO
					ENDIF
				ELSE
					DO LT=1,NTMUSK+1
