...
!IF (xyz==1) THEN
    ...
    ALLOCATE(QSJ2(NC,3))
    ALLOCATE(FracS(NC,3))
    ALLOCATE(DEPTREC(NC,3), DEPT(NC,3), EROSTREC(NC,3), EROT(NC,3), CARGM(NC,3),CSdep(NC,3))
...
!!RUGMAN = 0.030	    !@ constante para todo trecho, como em PARCUNGE                     !@ HUGO FAGUNDES...agora esse valor é lido do CELL.hig
!@ ***************************************************************************************
...
VFL1        = 0. !@ DCB_HD_Sed - VOLUME DE ÁGUA NA PLANÍCIE NO TEMPO T
VFL2        = 0. !@ DCB_HD_Sed - VOLUME DE ÁGUA NA PLANÍCIE NO TEMPO T+1
CFL1        = 0. !@ DCB_HD_Sed - CONCENTRAÇÃO NA PLANÍCIE NO TEMPO T
CFL2        = 0. !@ DCB_HD_Sed - CONCENTRAÇÃO NA PLANÍCIE NO TEMPO T+1
DFL         = 0. !@ DCB_HD_Sed - DEPÓSITO DE SILTE E ARGILA NA PLANÍCIE
inFL        = 0. !@ DCB_HD_Sed - ACUMULADO DAS CARGAS QUE ENTRAM NAS PLANÍCIES
outFL       = 0. !@ DCB_HD_Sed - ACUMULADO DAS CARGAS QUE SAEM DA PLANÍCIE
BalQFL      = 0. !@ DCB_HD_Sed - BALANÇO DE VAZÃO ENTRANDO E SAINDO NAS PLANÍCIES
VTOTAL      = 0. !@ DCB_HD_Sed - VOLUME DE ÁGUA NOS RIOS E NAS PLANÍCIES
Qlim        = 0.01
!@ ***************************************************************************************
...
!##############################################################################################LBF08/2018
!Trocou NTR por NC
!DO IC = 1, NC                      !@ DCB_HD_Sed
!##########################################################################################################
! ERA CellTr(TrCell(IC)) = IC         !@ DCB_HD_Sed, alteração abaixo:

    !CellTr = IC         !@ DCB_HD_Sed
!##############################################################################################LBF08/2018
...
!	IF(IB>82 .OR. IB<57)CYCLE !@para o madeira LBF 02/2019
    !@ DCB 30/04/1011 ##################################################################################


    !@ DCB sed 20_09_2012 ##############################################################################
     if (IB>SUBini-1 .AND. IB<SUBfim+1) then
   !  if (IB>56 .AND. IB<83) then !@para o madeira LBF 02/2019
	...
	If(hdFLAG0==0)then
		...
		HmJ = ((RUGMAN(IC)*QJ2(IC))/(BRIO(IC)*SfTREC**0.5))**(3.0/5.0)
	...
	ELSE
	    ...
	    !@ Profundidade média na seção de jusante por Manning assumindo Raio = HmTREC (m)
		!HmJ = Hfl(iC)
		HmJ = ((RUGMAN(IC)*abs(QJ2(IC)))/(BRIO(IC)*SfTREC**0.5))**(3.0/5.0) 
!        HmJ = ((RUGMAN(IC)*QJ2(IC))/(BRIO(IC)*SfTREC**0.5))**(3.0/5.0)
	    ...
	    !@ Velocidade média na seção de jusante (m/s)
	    VmJ = 0.0
        IF (QJ2(IC) > Qlim) VmJ = QJ2(IC)/(BRIO(IC)*HmJ)
