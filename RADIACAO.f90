	...
	IF (flagaclimed == 1) then			! from insolation
        if (CRU_INDEX==1) then
            ...
            SSUP=STO*(0.25+0.5*SOLX/HIM) 	!Sunshine data in hours per day for INMET Data (eq. 4.2.6 from Handbook Hydrology)
        endif
    ELSE		!from incident shortwave on terr surface data		
		!SSUP=SOLX	
         SSUP=STO*(0.25+0.5*SOLX/HIM) !Climate daily data
    ENDIF
