 !*********************************************************************************
	...
	! Opens mini.gtp:
	OPEN(FILHIG,FILE='.\input\MINI.gtp',STATUS='OLD',ACTION='READ')
	READ(FILHIG,77) !RP
	IBANT=1
	IEXUT=-999999
...
!LBF        QREF(I)=0.333*ACUR(I)**0.7 !Calculates reference initial discharge
        QREF(I)=30*ACUR(I)/1000.
	...
	!IBAC=1
	write(*,*)''
	!write(*,*)'Minimum reach lenght set to 2.0 km'
	!! Correct length of main river. Minimum length = 2.5 km:   
	!do iC=1,nC
	!	SRIO(iC)=max(SRIO(iC),2.0)
	!enddo
