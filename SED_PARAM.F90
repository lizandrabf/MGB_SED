...
INTEGER iUSO,iCEL,k,i

REAL Fagros, Fargila, Forg, Fareia
IF (xyz==1) THEN
    ALLOCATE(Kusle(NB,NU), Cusle(NB,NU), Pusle(NB,NU), Rgros(NB,NU), Ksdr(NB,NU)) !@ Ksdr(NU)
    ALLOCATE(Mareia(NB,NU), Msilte(NB,NU), Margila(NB,NU), Morg(NB,NU), Mrocha(NB,NU))
    ALLOCATE(LSAcu(NC,NU), NPXU(NC,NU), SDR(NC,NU+1)) !@ SDR(NC,NU+1)  
    ALLOCATE(PSC(NC,4), SLC(NC), QSC(NC), QSAUX(NC,NU+2))
    ALLOCATE(SLU(NC,IU))
    ALLOCATE(QSU(NC,NU), PSU(NC,NU), SLXU(NU))
    ALLOCATE(CAREIA(NMAXHID,NT), CSILTE(NMAXHID,NT), CARGIL(NMAXHID,NT)) !CONCENTRAÇÕES PARA GRAVAÇÃO
    ALLOCATE(CSSCAL(NMAXHID,NT,10),SEDstore(NMAXHID,NT,10))  !CSS A SER COMPARADA COM DADOS OBSERVADOS, 10 diz respeito ao número de base de dados que se tá usando (SSC, TSS, Reflectance, Turbidity, etc.)
ENDIF
...
DO IB=1,NB
    ...
    !	Morg(iUSO) = 1.72*Morg(iUSO)
	    ...
	    IF (Kusle(IB,iUSO)==-1.) THEN
		    ...
		    Fagros = 0.2 + 0.3*exp(-0.0256*Mareia(IB,iUSO)*(1. - Msilte(IB,iUSO)/100.))
