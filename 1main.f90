...
!*********************************************************************************
!***************MODELO HIDROLÓGICO DE GRANDES BACIAS (MGB)************************
!                                   +
!**************MÓDULO DE EROSÃO E TRANSPORTE DE SEDIMENTOS************************
...
!*********************************************************************************
!
!  PROGRAM MGB_Inercial (1main.f90) is the main routine from MGB.
...
!
!    From there on MGB was modified for research and project purposes.
!    The current package is the official version of MGB.
!
...
!   *Module SED_VARS    !module containing the variavçe of the sediment model       Hugo Fagundes  01.07.19
...
!   *Subroutine	ALLOCA_VARS_SED !Allocate the Sediment model main variables
...
!   
!    20 July 2019
!    By: Hugo de Oliveira Fagundes and Lizandra Broseghini Föeger
!    
!  Authors:
!
...
!  Calls used libraries and modules 
	...
	USE SED_VARS    !module containing the variables of the sediment model         HUGO FAGUNDES 28.06.19
    
!  Variables declaration 

	IMPLICIT NONE !States that non declared variables cannot be used
	INTEGER JULDAY, k !Variable used for day of the year calculation on a Julian calendar
...
!  Write the model name in the black box
    WRITE(*,*)'Modelo Hidrologico de Grandes Bacias: MGB-IPH'
    WRITE(FILLOG,*)'Modelo Hidrologico de Grandes Bacias: MGB-IPH'
...
!  Call subroutines related to input files that are readen only once reading and MGB-IPH variables allocation
	...
	CALL LECLIMED !Subroutine for reading the file with climatology data (File: MEDIAS.CLM)
...
!*****************************************************************
	! Hiydrodynamic modeling:
	!hdFLAG=1 !Use 0 to deactivate Hydrodynamic modeling; Use 1 to activate.
	!If(hdFLAG(NC)==1)THEN
    If(IND_FLOODROUTING==1)THEN 
        hdFLAG=1 !If flood routing method is Inertial, hdFlag=1
        CALL LEFLOODPLN !Subroutine for reading the file with a table containing a table of water-depth vs. area at each catchment obtained by DEM preprocessing (FileName: COTA_AREA.FLP)
    ELSE
        hdFLAG=0
    ENDIF
    hdFLAG0=sum(hdFLAG) 
...
!  Call subroutines that calculates MicroBasins, rivers channels, and floodplains characteristics
    WRITE(*,*)'Calculating river geometry informations...'
    WRITE(FILLOG,*)'Calculating river geometry informations...'
    ...
    !    ******************** Hugo Fagundes 03.07.19 ******************** 
    IF (SED_INDEX==1) THEN
        CALL ALLOCA_VARS_SED(0) !Allocate the sediment model main variables
        WRITE(*,*)'Allocating variables...'
	    WRITE(FILLOG,*)'Allocating variables...'
        CALL LEOBSed !Subroutine for reading the file observed sediment data (File with extension .txt)
        WRITE(FILLOG,*)'SED OBS OK'
        IF (ICALIB.EQ.0) THEN
            xyz=1 !Auxiliar variable for sediment automatic calibration     Hugo Fagundes 03.07.19
            CALL SED_PARAM !Reading/calculating fixed MUSLE parameters
            WRITE(*,*)'Reading fixed MUSLE parameters file...'
	        WRITE(FILLOG,*)'Reading fixed MUSLE parameters file...'
        ENDIF
    ENDIF
        !    ******************** Hugo Fagundes 03.07.19 ******************** 
...
!  Openning of MGB Input and Output files that are read and written during the simulation
	...
	OPEN(FILAJU,FILE=OUTPUT_DIRECTORY // 'AJUSTE.fob',STATUS='UNKNOWN') !Output file with objective functions values for the locations with flows observed data
    OPEN(FILAJUS,FILE=OUTPUT_DIRECTORY // 'AJUSTESED.fob',STATUS='UNKNOWN') !Output file with objective functions valuess for the locations with sediment observed data
!  End of openning MGB main Input and Output files that are read and written over the simulation
...
!  Informs the model general setup
    ...
    WRITE(FILLOG,'(A30,I8)')'Number of observation gauges (discharge):',NOBS
    WRITE(FILLOG,'(A30,I8)')'Number of substitution gauges (discharge):',NUMSUBST
    DO K=1,AUXSS
        WRITE(FILLOG,'(A30,I8)')'Number of observation gauges (sediment):',NOBSS(K)      !Hugo Fagundes 28.06.19
    ENDDO
    WRITE(FILLOG,'(A30,I8)')'Number of climate gauges:',NCLI
    WRITE(FILLOG,*)'ICALIB (simulation=0; autocalibration=1; forecasting=2; Sed autocalibration=3)' !Hugo Fagundes 18/07/2019
    WRITE(FILLOG,'(A30,I8)')'ICALIB :',ICALIB
    write(*,*)''
    WRITE(FILLOG,'(A40,I8)')'Reservoir Flag (0=no; 1=yes):',RESERV_INDEX
    WRITE(FILLOG,'(A40,I8)')'Inertial routing Flag (0=no; 1=yes):',IND_FLOODROUTING
    WRITE(FILLOG,'(A40,I8)')'Sediment module Flag (0=no; 1=yes):',SED_INDEX         !Hugo Fagundes 28.06.19
    ...
    WRITE(*,'(A30,I8)')'Number of observation gauges (discharge):',NOBS
    WRITE(*,'(A30,I8)')'Number of substitution gauges (discharge):',NUMSUBST
    WRITE(*,'(A30,I8)')'Number of observation gauges (sediment):',NOBSS(1)      !Hugo Fagundes 28.06.19
    WRITE(*,'(A30,I8)')'Number of climate gauges:',NCLI
     WRITE(*,*)'ICALIB (simulation=0; autocalibration=1; forecasting=2; Sed autocalibration=3)' !Hugo Fagundes 18/07/2019
    ...
    WRITE(*,'(A40,I8)')'Inertial routing Flag (0=no; 1=yes):',IND_FLOODROUTING
    WRITE(*,'(A40,I8)')'Sediment module Flag (0=no; 1=yes):',SED_INDEX !Hugo Fagundes 28.06.19
...
!  Select the mode of MGB-IPH run: Simulation (ICALIB=0), Automatic Calibration (ICALIB=1), Forecasting (ICALIB=2)
	...
	CASE (1) !Hydrological Automatic Calibration run
		WRITE(*,*)'AUTOMATIC CALIBRATION MODE - HIDROLOGICAL MODEL' !Warns about the chosen mode
		WRITE(FILLOG,*)'AUTOMATIC CALIBRATION MODE - HIDROLOGICAL MODEL' !Warns about the chosen mode
	...
	CASE (2) !Forecasting run
		...
		CALL PREVISAO !Subroutine that calls a forecasting run
        
    !    ******************** Hugo Fagundes 03.07.19 ********************    
    CASE (3)!Sediment Automatic Calibration run
        WRITE(*,*)'AUTOMATIC CALIBRATION MODE - SEDIMENT MODEL' !Warns about the chosen mode
		WRITE(FILLOG,*)'AUTOMATIC CALIBRATION MODE - SEDIMENT MODEL' !Warns about the chosen mode
		CALL LeCalib_SED !Subroutine that reads sediment automatic calibration parameters
		CALL ALLOCA_CALIB(0) !Subroutine that allocates sediment automatic calibration input files
		CALL CALIBRA_SED !Subroutine that alls a sediment model automatic calibration run
		CALL ALLOCA_CALIB(1) !Subroutine that cleans sediment automatic calibration input files
    !    ******************** Hugo Fagundes 03.07.19 ******************** 
...
!  After running the model, finish the time count, close all files, and clean all variables 
	...
	CLOSE (FILAJUS) !Close output file with objective functions values for the locations with sediment observed data
	CALL ALLOCA_VARS(1) !Cleans main variables
    CALL ALLOCA_VARS_SED(1) !Cleans main variables
	
! End of closings and cleanings

! Inform the user that the model run is over and informations about total processing time

	WRITE(*,*)'MGB ENDED SUCCESSFULLY'
	WRITE(FILLOG,*)'MGB ENDED SUCCESSFULLY'
	print *, char(7)
