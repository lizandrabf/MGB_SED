    ...
    OPEN(FILRESCALIB, FILE='.\output\RESULCALIB.TXT', STATUS='UNKNOWN') !ARQUIVO DE SAÍDA COM OS RESULTADOS DA CALIBRAÇÃO           !HUGO FAGUNDES 02/10/19
    ...
    PAR(1,1)=11.8   !Alfa da equação MUSLE (Williams, 1975) CHUTE INICIAL DOS PARAMETROS ALFA
    PAR(2,1)=0.56   !Beta da equação MUSLE (Williams, 1975) CHUTE INICIAL DOS PARAMETROS BETA
    PAR(3,1)=1.0    !Chute inicial para mudança proporcional do parâmetro TKS
    DO I=1,NB-1
        PAR(3*I+1,1)=11.8
        PAR(3*I+2,1)=0.56
        PAR(3*I+3,1)=1.00
    ENDDO
    !PAR TERÁ DIMENSOES (NUMERO DE PARAMETROS X NUMERO DE INDIVIDUOS)
    DO I=2,NS
	    DO L=1,NPAR
            PPAR(L,I)=RAN1(ISEED)
        ...
        ENDDO
    ENDDO
    xyz=1 !Variável auxiliar para calibração
    ...
    CALL SED_INICIAL
	CALL MODELO
    !!!CALL SED_Fob_Calib !FUNÇÃO OBJETIVO PARA CALIBRAÇÃO DO MODELO DE SEDIMENTOS
	...
	DO I=1,NS   !HOF 09/01/2019
		...
		!********************************************************************************************************
		!!!CALL SED_Fob_Calib !FUNÇÃO OBJETIVO
...
79		FORMAT(3I8,8F12.4)
	...
	!!!!!************************************************************************************************
 !!!!   WRITE(FILRESCALIB,*)AUXCALIBSS !ESCREVE QUAL CONJUNTO DE DADOS ESTA SENDO USADO PARA CALIBRAÇÃO
 !!!!   DO KS=1,NS
 !!!!       PARX=PAR(:,KS)
 !!!!       !ESCREVE OS PARÂMETROS ALFA E BETA
 ...
 !!!!       CALL SED_CALIB_MODELO
	!!!!	!********************************************************************************************************
 !!!!
 !!!!       CALL SED_Fob_Calib !FUNÇÃO OBJETIVO PARA OS SEDIMENTOS PARA TODOS OS CONJUNTOS DE DADOS
 !!!!           
 !!!!       !ARMAZENA OS DADOS (PODERIA APENAS ESCREVER, SE NÃO QUISER ARMAZENAR, COMENTAR AS LINHAS ABAIXO
 ...
 !!!!
 !!!!       !ESCREVE OS PARÂMETROS ALFA E BETA E OS RESULTADOS DAS FUNÇÕES OBJETIVO        
 !!!!       WRITE(FILRESCALIB,73)(PARX(K),K=1,NPAR),(RESULTSS(K,KS),K=1,8) !8 numero de funções objetivo  !(RESULTSS(K,KS),K=1,NF*AUXSS) HOF 09/01/2019
 !!!!   ENDDO  
    CLOSE (FILRESCALIB)
    73 FORMAT(F10.3)
